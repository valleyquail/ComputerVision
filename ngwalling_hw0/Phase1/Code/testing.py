...
# lm_filters = make_leung_malik(size=49) + make_leung_malik(size=49,sigmas=(np.sqrt(2),2,2*np.sqrt(2),4))
# garbor_filters = plot_garbor(size=49)
#
filters = dog_filters # + lm_filters + garbor_filters

texton = generate_texton_map(imgs[2], filters, show=True)
#
# brightness = generate_brightness_map(imgs[9], show=True)
# color = generate_color_map(imgs[9], show=True)
#
...
half_disks_1 = make_half_disk(7, True)
half_disks_2 = make_half_disk(10, True)
half_disks_3 = make_half_disk(16, True)
#
...
#
# sobel = cv2.cvtColor(sobels[9], cv2.COLOR_BGR2GRAY)
# canny = cv2.cvtColor(cannys[9], cv2.COLOR_BGR2GRAY)
#
