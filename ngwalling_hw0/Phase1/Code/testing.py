...
lm_filters = make_leung_malik(size=49, show=True) + make_leung_malik(size=49,sigmas=(np.sqrt(2),2,2*np.sqrt(2),4))
...
# #
# brightness = generate_brightness_map(imgs[2], show=True)
# color = generate_color_map(imgs[2], show=True)
#
...
# #
# sobel = cv2.cvtColor(sobels[2], cv2.COLOR_BGR2GRAY)
# canny = cv2.cvtColor(cannys[2], cv2.COLOR_BGR2GRAY)
#
